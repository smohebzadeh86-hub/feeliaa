...
|---|---|---|---|---|---|
| GET | `/api/clients` | — | `{clients:[{id, code, alias, created_at, status, status_reason, category, gender, pinned_at, session_count, last_session_date}]}` مرتب با `created_at DESC` | — | UI |
...
|---|---|---|---|---|---|
| GET | `/api/stt/check` | — | همیشه 200: `{ok:true, code:"mint-ok", proxy:{ok, code?}, websocket_url}` یا `{ok:false, code, error, proxy?}` | — | UI (preflight، غیرمسدودکننده) |
