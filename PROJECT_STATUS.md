...
- `8a2a82d` UIِ رضایت بدونِ متن/دکمه‌ی لغو (deployشده) · `91ad119` سندِ اعمالِ 025 رویِ production · `58e12ee` پرونده از آپلودِ غیرفعال پشتِ
  `UPLOAD_CASE_FILE_INACTIVE` (خاموش؛ deployشده) · `9be0cec` رفعِ MaxListeners در `assembleUpload` (deployشده) · `baa9165` تلاشِ دوباره‌ی خطایِ گذرایِ LLM
  (**deploy نشده** — رویِ production تا روشن‌شدنِ سوئیچ بی‌اثر است) · `48b8125` اسناد.
- **روش:** محتوایِ هر commit دقیقاً مشخص و مستقیم در index گذاشته شد (نسخه‌هایِ ۳ و ۴ = فایل‌هایِ دانلودشده از production)؛ حالتِ staged ِ commitهایِ ۳/۴/۵
  در پوشه‌ی جدا (`checkout-index`) تست شد: `test:up` 37/38/41 PASS، `tsc` تمیز. یک `stash --keep-index` برایِ تستِ commitِ ۳ در `pop` تداخل داد؛ فایل‌ها از
  خودِ stash بازگردانده شدند، حالتِ نهایی دوباره تست شد (41/41) و stash فقط بعد از تأییدِ برابری با HEAD حذف شد.
- بیرون ماند (مثلِ قبل): `.claude/`، tarها، `package-lock.json`، `server-deploy/`، `soniox.html`. **عامل:** این نشست.
