...
// ⭐ فیکسِ باگِ واقعیِ کشف‌شده: WebM/Opus یک container با state ست — هر SonioxEngine
// تازه یک decode context تازه‌ست که فقط با هدرِ خودِ همون MediaRecorder معتبره.
// وقتی موتور جدید ساخته می‌شه (reconnect یا resume)، کلاینت هم مجبوره MediaRecorder
// تازه بسازه (هدرِ تازه)، پس شماره‌گذاریِ seq/nextExpected این سمت هم باید صفر بشه —
// وگرنه بایت‌های ته‌مانده‌ی استریمِ قبلی به‌عنوانِ اولین بایتِ موتورِ جدید فرستاده می‌شه
// و Soniox با «Audio decode error» ردش می‌کنه.
export function resetOrdering(r: P1Record) {
  r.nextExpected = 1;
  r.buffer.clear();
  r.forwarded.clear();
  r.forwardedOrder.length = 0;
  clearReorderTimer(r);
}
